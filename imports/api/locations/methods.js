...
export const createLocation = new ValidatedMethod({
  ...
  validate: Locations.simpleSchema()
    .pick(["name"])
    .validator(),
  run({ name }) {
    if (this.connection) {
      ...
      if (!user.isTAOrAbove()) {
        throw new Meteor.Error(
          "locations.createLocation.unauthorized",
    ...
    if (location) {
      throw new Meteor.Error(
        "locations.createLocation.alreadyExists",
...
export const updateLocation = new ValidatedMethod({
  ...
  run({ locationId, name }) {
    ...
    if (!user.isTAOrAbove()) {
      throw new Meteor.Error(
        "locations.updateLocation.unauthorized",
...
export const deleteLocation = new ValidatedMethod({
  ...
  run({ locationId }) {
    ...
    if (!user.isTAOrAbove()) {
      throw new Meteor.Error(
        "locations.deleteLocation.unauthorized",
