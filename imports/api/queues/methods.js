...
export const createQueue = new ValidatedMethod({
  ...
  validate: Queues.simpleSchema()
    .pick(["name", "courseId", "locationId", "scheduledEndTime"])
    .validator(),
  run({ name, courseId, locationId, scheduledEndTime }) {
    ...
    if (
      ...
      throw new Meteor.Error(
        "queues.createQueue.unauthorized",
...
export const cutoffQueue = new ValidatedMethod({
  ...
  run({ queueId }) {
    ...
    if (
      ...
      throw new Meteor.Error(
        "queues.cutoffQueue.unauthorized",
...
export const shuffleQueue = new ValidatedMethod({
  ...
  run({ queueId }) {
    ...
    if (
      ...
      throw new Meteor.Error(
        "queues.shuffleQueue.unauthorized",
    ...
    if (queue.isCutoff()) {
      throw new Meteor.Error(
        "queues.shuffleQueue.cannotShuffleCutoffQueue",
    ...
    const shuffledTicketIds = inactiveTicketIds.concat(
      _.shuffle(activeTicketIds)
    );
...
export const restrictToSession = new ValidatedMethod({
  ...
  run({ queueId, name, userAgent, secret }) {
    ...
    if (
      ...
      throw new Meteor.Error(
        "queues.restrictToSession.unauthorized",
