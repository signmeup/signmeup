...
import "./analytics-logs.html";

Template.AnalyticsLogs.onCreated(function onCreated() {
  this.status = new ReactiveVar("Empty");
  this.downloadUrl = new ReactiveVar("");
  this.fileName = new ReactiveVar("");
});

Template.AnalyticsLogs.onRendered(() => {
  $(".js-logs-datepicker").datepicker();
});

Template.AnalyticsLogs.helpers({
  isDownloadReady() {
  ...
  "click .js-logs-prepare"(event, templateInstance) {
    ...
    Meteor.call(
      ...
      (err, result) => {
        ...
        templateInstance.fileName.set(
          [this.course.name, type, start, end].join("_")
        );
