...
Template.QueueMoreDropdown.events({
  ...
  "click .js-shuffle-tickets"(event) {
    ...
    if (this.queue.isCutoff()) {
      alert(
        "Sorry, cutoff queues cannot be shuffled. Resume the queue to proceed."
      );
      return;
    }

    const sure = prompt(
      "Are you sure you want to shuffle all tickets? If yes, type 'SHUFFLE' in the input below."
    );
    if (sure === "SHUFFLE") {
      shuffleQueue.call(
        {
  ...
  "click .js-delete-all-tickets"(event) {
    ...
    const sure = prompt(
      "Are you sure you want to delete all tickets? If yes, type 'DELETE-ALL' in the input below."
    );
    if (sure === "DELETE-ALL") {
